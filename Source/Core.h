...
public:
	...
	void init(ID3D12Device* device, int numDescriptors = 1024) // [cite: 43]
	{
		D3D12_DESCRIPTOR_HEAP_DESC desc = {};
		desc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV; // [cite: 46]
		desc.NumDescriptors = numDescriptors;
		desc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE; // [cite: 48]

		device->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&heap));

		cpuHandle = heap->GetCPUDescriptorHandleForHeapStart(); // [cite: 49]
	...
	D3D12_CPU_DESCRIPTOR_HANDLE getNextCPUHandle()
	{
		...
		ret.ptr += (UINT64)used * incrementSize;
		used++;
		return ret;
	}
};
...
public:
	...
	DescriptorHeap srvHeap; // [cite: 89]
